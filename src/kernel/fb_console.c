...
static void draw_char(char c, int x, int y) {
    ...
    /* Draw font rows (8 rows) */
    for (i = 0; i < 8; i++) {
        row = font[(int)c][i];
        for (j = 0; j < 8; j++) {
            if (row & (0x80 >> j)) {
                gfx_set_pixel(px + j, py + i, fg_color);
            } else {
                gfx_set_pixel(px + j, py + i, bg_color);
            }
        }
    }
    
    /* Draw 4 extra rows of background for spacing */
    for (i = 8; i < CHAR_HEIGHT; i++) {
...
static void scroll(void) {
    ...
    /* Redraw all lines shifted up */
    for (y = 0; y < CONSOLE_ROWS - 1; y++) {
        for (x = 0; x < CONSOLE_COLS; x++) {
            /* Copy pixel data from line below */
            int px = x * CHAR_WIDTH;
            int src_y = (y + 1) * CHAR_HEIGHT;
            int dst_y = y * CHAR_HEIGHT;
            int i, j;
            
            for (i = 0; i < CHAR_HEIGHT; i++) {
                for (j = 0; j < CHAR_WIDTH; j++) {
                    uint32_t pixel = gfx_get_pixel(px + j, src_y + i);
                    gfx_set_pixel(px + j, dst_y + i, pixel);
                }
            }
        }
    }
